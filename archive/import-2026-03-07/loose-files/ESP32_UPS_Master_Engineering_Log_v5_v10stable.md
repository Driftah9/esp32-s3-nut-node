...
### Validation Run — v10 (Milestone 7) — 2026-03-05

**Build outcome:** ✅ Compiles after adding forward declaration for `wifi_init_softap()` (fixes implicit declaration / static mismatch).  
**WiFi SoftAP:** ✅ AP started; client associated; DHCP lease issued (**client got 192.168.4.2**).  
**HTTP server:** ✅ Started; `/status` handler installed; browser GET `/status` served successfully.  
**USB pipeline:** ✅ Enumeration, interface claim, interrupt-IN reader, change-only reports, and decode all continue working while HTTP is active.

**Observed expected behavior:**
- `GET /` returns **404** (no handler) — acceptable for now.
- `GET /favicon.ico` returns **404** (no handler) — acceptable for now.
- NUT-ish decode/state continues updating in parallel with WiFi + HTTP traffic.

**Representative decoded values during run:**
- `battery.charge` ≈ 99%
- `battery.runtime` ≈ 35640 s
- `input.utility.present` = 1 (OL)
- `flags` = 0x0000000D


## Next Steps (Planned)

### Milestone 8 (v11): Minimal NUT protocol service (LAN)
Goal: allow a NUT client to query UPS variables over the network (starting read-only).

Planned approach (incremental):
1. Keep v10 HTTP `/status` as a debugging backstop (do not remove).
2. Implement a **TCP server** on the NUT default port **3493**.
3. Implement a minimal subset of the upsd protocol sufficient for:
   - `VER`
   - `LIST UPS`
   - `GET VAR <ups> <var>` for core vars:
     - `battery.charge`
     - `battery.runtime`
     - `input.utility.present`
     - `ups.status`
     - `ups.flags` (or expose as driver/debug var)
   - Optionally `LIST VAR <ups>` (for client convenience)
4. Hardcode a single UPS name initially (e.g. `ups`) and a single client (no auth) on LAN.
5. Add basic throttling + robustness: handle malformed commands, partial lines, and disconnects cleanly.

Validation targets:
- From a PC on the SoftAP network, connect via `telnet 192.168.4.1 3493` and confirm responses.
- From a Linux host, run `upsc ups@192.168.4.1` (or equivalent) and confirm variables.
- Confirm USB decode continues while NUT polling occurs.


## Verified Working Code (v10)

```c
/* main.c CURRENT VERSION: v10
 * Version tracking:
 *   v8 (2026-03-05): Milestone 5 start: maintain decoded UPS state + NUT-style status logs.
 *                   Disable full HID report descriptor dump by default (V7 control submit ESP_ERR_INVALID_ARG).
 *                   Keep usb_host_lib_handle_events() pump + async client callback + change-only IN logging.
 *   v9 (2026-03-05): Milestone 6 start: track remaining common RIDs (0x06/0x14/0x21),
 *   v10 (2026-03-05): Milestone 7 start: add WiFi SoftAP + HTTP /status JSON endpoint (read-only).
 *                   add RID counters + last-seen timestamps, add flags bit breakdown,
 *                   and add state freshness timestamps (kept out of change-detection to avoid spam).
 *
 * Notes:
...
/* =========================================================
 * Milestone 7 (v10): Minimal WiFi SoftAP + HTTP status endpoint
 *
 * Default network for quick testing:
 *   SSID: ESP32_UPS_MON
 *   PASS: upsmonitor
 *   IP:   192.168.4.1 (ESP-IDF default SoftAP netif)
 *
 * Endpoint:
 *   GET http://192.168.4.1/status  -> JSON snapshot (read-only)
 *
 * Notes:
 * - This uses SoftAP so you don't need to touch your LAN/router to validate.
 * - Later milestone can add STA mode + configurable credentials/IP.
 * ========================================================= */

#define WIFI_AP_SSID       "ESP32_UPS_MON"
#define WIFI_AP_PASS       "upsmonitor"
...
static void wifi_init_softap(void)
{
    ...
    ESP_LOGI(TAG, "WiFi SoftAP started: SSID=%s PASS=%s", WIFI_AP_SSID, WIFI_AP_PASS);
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Milestone 5: state + NUT-ish status logs");
    ESP_LOGI(TAG, "Milestone 6: track 0x06/0x14/0x21 + counters + freshness");
    ESP_LOGI(TAG, "Milestone 7: WiFi SoftAP + HTTP /status JSON");
    ESP_LOGI(TAG, "ESP-IDF %s", IDF_VER);
    ESP_LOGI(TAG, "main.c CURRENT VERSION: v10");
    ESP_LOGI(TAG, "=======================================");
 ...
 *
 * v10 (Milestone 7)
 * - [ ] Confirm SoftAP comes up (ESP32_UPS_MON / upsmonitor)
 * - [ ] Confirm GET http://192.168.4.1/status returns JSON while USB streaming is active
 * - [ ] Confirm detach/reattach does not break HTTP responsiveness
 *
 * v11 (Milestone 8)
 * - [ ] Implement minimal NUT TCP subset on port 3493 (VER, LIST UPS, LIST VAR, GET VAR, USERNAME/PASSWORD, QUIT)
 *
