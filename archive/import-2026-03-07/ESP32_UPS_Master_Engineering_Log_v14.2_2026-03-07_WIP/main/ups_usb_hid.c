/*============================================================================
 ...
 R5  v14.2 basic manufacturer/model identity defaults for metadata vars
...
static void log_device_descriptor(usb_device_handle_t dev)
{
    ...
    /* Best-effort identity until USB string descriptor reads are implemented. */
    if (dd->idVendor == 0x051d) {
        mfr = "APC";
    }
    if (s_hid_rpt_len == 1049) {
        model = "APC HID 1049-byte profile";
    } else if (s_hid_rpt_len == 1133) {
        model = "APC HID 1133-byte profile";
    }

    ups_state_set_usb_identity(s_vid, s_pid, s_hid_rpt_len, mfr, model, "UNKNOWN");
}
...
static esp_err_t parse_active_config(usb_device_handle_t dev)
{
    ...
    for (uint16_t i = 0; i + 2U <= total;) {
        ...
        if (bType == USB_B_DESCRIPTOR_TYPE_INTERFACE && bLength >= sizeof(usb_intf_desc_t)) {
            ...
            const char *mfr = (s_vid == 0x051d) ? "APC" : "UNKNOWN";
            const char *model = "UNKNOWN";
            if (s_hid_rpt_len == 1049) {
                model = "APC HID 1049-byte profile";
            } else if (s_hid_rpt_len == 1133) {
