/*============================================================================
 ...
 R3  v14.10 serves candidate telemetry variables when parser validates them
 R4  v14.2 adds metadata variables (device.*, driver.*) and compatibility aliases
...
#define DRIVER_NAME_STR "esp32-usbhid-apc"
...
static const char *meta_mfr(const ups_state_t *st) {
    return (st->manufacturer[0] ? st->manufacturer : "UNKNOWN");
}

static const char *meta_model(const ups_state_t *st) {
    return (st->product[0] ? st->product : "UNKNOWN");
}
...
static void nut_handle_line(app_cfg_t *cfg, int fd, nut_session_t *sess, char *line) {
    ups_state_t st; ups_state_snapshot(&st);
    (void)sess;
    ...
    if (strcasecmp(argv[0], "GET") == 0 && argc >= 4 && strcasecmp(argv[1], "VAR") == 0) {
        ...
        if      (strcmp(var, "battery.charge") == 0)        nut_sendf(fd, "VAR %s %s \"%u\"\n", ups, var, (unsigned)st.battery_charge);
        else if (strcmp(var, "battery.runtime") == 0)       nut_sendf(fd, "VAR %s %s \"%u\"\n", ups, var, (unsigned)st.battery_runtime_s);
        else if (strcmp(var, "input.utility.present") == 0) nut_sendf(fd, "VAR %s %s \"%u\"\n", ups, var, st.input_utility_present ? 1u : 0u);
        else if (strcmp(var, "ups.flags") == 0)             nut_sendf(fd, "VAR %s %s \"0x%08X\"\n", ups, var, (unsigned)st.ups_flags);
        else if (strcmp(var, "ups.status") == 0)            nut_sendf(fd, "VAR %s %s \"%s\"\n", ups, var, st.ups_status[0] ? st.ups_status : "UNKNOWN");
        else if (strcmp(var, "device.mfr") == 0)            nut_sendf(fd, "VAR %s %s \"%s\"\n", ups, var, meta_mfr(&st));
        else if (strcmp(var, "device.model") == 0)          nut_sendf(fd, "VAR %s %s \"%s\"\n", ups, var, meta_model(&st));
        else if (strcmp(var, "driver.name") == 0)           nut_sendf(fd, "VAR %s %s \"%s\"\n", ups, var, DRIVER_NAME_STR);
        else if (strcmp(var, "ups.mfr") == 0)               nut_sendf(fd, "VAR %s %s \"%s\"\n", ups, var, meta_mfr(&st));
        else if (strcmp(var, "ups.model") == 0)             nut_sendf(fd, "VAR %s %s \"%s\"\n", ups, var, meta_model(&st));
