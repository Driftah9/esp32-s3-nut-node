/*============================================================================
 ...
 R1  v14.8 scan + identify + claim + INT-IN logging (this package)

 IMPORTANT
 - Requires CONFIG_USB_HOST_ENABLED=y in sdkconfig / menuconfig
...
/*============================================================================
 ...
 - Claim HID interface and (optionally) stream interrupt IN reports (change-only)

 CURRENT STATE (v14.8 step)
 - Scans ANY attached USB device, logs VID:PID
 - Parses ACTIVE CONFIG DESCRIPTOR to locate:
    * HID interface (bInterfaceClass=0x03)
    * HID descriptor (0x21) -> report descriptor length
    * Interrupt IN endpoint -> EP addr + MPS
 - Claims the HID interface and can start interrupt-IN streaming (change-only)
 - Optional: best-effort HID report descriptor dump via control GET_DESCRIPTOR(0x22)

 NOTES (from prior validated work)
 - Use usb_host_get_device_descriptor() for VID/PID (fields differ from usb_device_info_t)
 - In ESP-IDF v5.3.1 transfer type is usb_transfer_t
 - Endpoint IN test: (ep->bEndpointAddress & 0x80)
 - XFERTYPE mask: USB_BM_ATTRIBUTES_XFERTYPE_MASK
 - NEW_DEV address must come from callback
 - No usb_host_transfer_cancel / usb_host_release_config_descriptor in v5.3.1
...
// Optional: set to 1 to dump the whole HID report descriptor (can be large)
#ifndef UPS_USB_ENABLE_REPORT_DESC_DUMP
#define UPS_USB_ENABLE_REPORT_DESC_DUMP 0
#endif

// Optional: set to 1 to start interrupt-IN reader (change-only logging)
...
static void log_device_descriptor(usb_device_handle_t dev)
{
    ...
    ESP_LOGI(TAG, "iMfr=%u iProduct=%u iSerial=%u cfg=%u",
             (unsigned)dd->iManufacturer, (unsigned)dd->iProduct, (unsigned)dd->iSerialNumber,
...
static esp_err_t parse_active_config(usb_device_handle_t dev)
{
    ...
    if (s_ep_in_addr == 0 || s_ep_in_mps == 0) {
        ESP_LOGW(TAG, "HID interface found but no interrupt IN endpoint detected");
        // still OK for identity-only stage
    }
    return ESP_OK;
}

// Best-effort HID report descriptor dump (can be large). Safe to disable.
static void maybe_dump_report_descriptor(usb_device_handle_t dev)
{
    ...
    // Cap to avoid huge allocations if device reports insane lengths
    const uint16_t want = (s_hid_rpt_len > 4096) ? 4096 : s_hid_rpt_len;
    ...
    int actual = 0;
    // bmRequestType=0x81 (IN, standard, device), bRequest=0x06 GET_DESCRIPTOR
...
// ---------- Interrupt IN Reader (change-only logging) ----------
...
static void intr_in_cb(usb_transfer_t *t)
{
    ...
    if (t->status == USB_TRANSFER_STATUS_COMPLETED && t->actual_num_bytes > 0) {
        ...
        if (changed) {
            ...
            int n = (len > 64) ? 64 : len;
            for (int i = 0; i < n; i++) {
                pos += snprintf(&line[pos], sizeof(line) - pos, "%02X%s", d[i], (i == n - 1) ? "" : " ");
            }
...
static void usb_client_task(void *arg)
{
    ...
    while (1) {
        ...
        if (s_dev_connected) {
            ...
            if (err == ESP_OK) {
                // Claim interface if found (even if no EP found, claim is OK for future control ops)
                if (s_hid_intf_num >= 0) {
                    err = usb_host_interface_claim(s_client, s_dev, (uint8_t)s_hid_intf_num, (uint8_t)s_hid_alt);
                    if (err == ESP_OK) {
                        ESP_LOGI(TAG, "Interface claimed (intf=%d alt=%d)", s_hid_intf_num, s_hid_alt);
                    } else {
        ...
        if (s_dev_gone) {
            ...
            if (s_dev) {
                ESP_LOGW(TAG, "Closing device handle");
...
#endif // CONFIG_USB_HOST_ENABLED

void ups_usb_hid_start(const app_cfg_t *cfg)
{
    (void)cfg;
#if CONFIG_USB_HOST_ENABLED
    // Run USB task on core0; keep other services on core1 if desired.
    xTaskCreatePinnedToCore(usb_client_task, "ups_usb", 6144, NULL, 20, NULL, 0);
    ESP_LOGI(TAG, "ups_usb_hid module started (scan+identify)");
#else
    ESP_LOGW(TAG, "USB Host disabled (CONFIG_USB_HOST_ENABLED=0). ups_usb_hid inactive.");
#endif
}
