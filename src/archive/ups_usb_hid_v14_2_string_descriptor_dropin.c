/* v14.2 drop-in for ups_usb_hid.c with USB string descriptor support */
...
static void set_identity_defaults(void)
{
    ...
    if (s_vid == 0x051d) strlcpy(s_mfr, "APC", sizeof(s_mfr));
    else strlcpy(s_mfr, "UNKNOWN", sizeof(s_mfr));
...
static bool fetch_string_desc_ascii(usb_device_handle_t dev, uint8_t index, char *out, size_t out_sz)
{
    ...
    esp_err_t err = usb_control_get_descriptor(
        ...
        0x80,
        0x06,
        (uint16_t)((USB_DESC_TYPE_STRING << 8) | index),
        0x0409,
        buf, sizeof(buf),
        &actual
    );

    if (err != ESP_OK || actual < 2) return false;
    if (buf[1] != USB_DESC_TYPE_STRING) return false;

    int in_len = buf[0];
    if (in_len > actual) in_len = actual;
    if (in_len < 2) return false;

    size_t pos = 0;
    for (int i = 2; i + 1 < in_len && pos + 1 < out_sz; i += 2) {
        uint16_t ch = (uint16_t)buf[i] | ((uint16_t)buf[i + 1] << 8);
...
static void load_usb_strings(usb_device_handle_t dev)
{
    ...
    if (err != ESP_OK || dd == NULL) {
        ESP_LOGW(TAG, "Cannot load USB strings: device descriptor unavailable");
        return;
    }

    bool got_mfr = fetch_string_desc_ascii(dev, dd->iManufacturer, s_mfr, sizeof(s_mfr));
    bool got_product = fetch_string_desc_ascii(dev, dd->iProduct, s_product, sizeof(s_product));
    bool got_serial = fetch_string_desc_ascii(dev, dd->iSerialNumber, s_serial, sizeof(s_serial));

    if (!got_mfr && s_vid == 0x051d) strlcpy(s_mfr, "APC", sizeof(s_mfr));
    else if (!got_mfr) strlcpy(s_mfr, "UNKNOWN", sizeof(s_mfr));

    if (!got_product) strlcpy(s_product, "UNKNOWN", sizeof(s_product));
    if (!got_serial) strlcpy(s_serial, "UNKNOWN", sizeof(s_serial));
...
static esp_err_t parse_active_config(usb_device_handle_t dev)
{
    ...
    if (s_hid_intf_num < 0) return ESP_ERR_NOT_FOUND;
    if (s_ep_in_addr == 0U || s_ep_in_mps == 0U) ESP_LOGW(TAG, "HID interface found but no interrupt IN endpoint detected");
    return ESP_OK;
}
