/*============================================================================
 ...
 - Fetch and inspect HID report descriptor
 ...
 R6  v14.11 cached USB string descriptor reader
 R7  v14.15 full drop-in with descriptor walker integration for v14.5

 NOTES
 - This file assumes the project also contains:
     ups_hid_parser.c/.h
     ups_hid_descriptor.c/.h
     ups_state.c/.h
 - The HID report descriptor is fetched with usb_control_get_descriptor()
   and then passed to ups_hid_descriptor_map_log() for structured logging.
...
#define UPS_USB_ENABLE_REPORT_DESC_DUMP 1
#endif
...
{
    if (s_hid_intf_num < 0 || s_hid_rpt_len == 0U) {
        ESP_LOGW(TAG, "Skip report descriptor fetch (missing interface or length)");
        return;
    }
    ...
    if (actual <= 0) {
        ESP_LOGW(TAG, "HID Report Descriptor fetch returned no data");
        heap_caps_free(buf);
        return;
    }

    ESP_LOGI(TAG, "HID Report Descriptor received: %d bytes", actual);
    ESP_LOGI(TAG, "===== HID REPORT DESCRIPTOR BEGIN =====");
    ...
    ESP_LOGI(TAG, "Walking HID report descriptor for UPS mapping...");
    ups_hid_descriptor_map_log(buf, (size_t)actual);
...
{
    (void)cfg;
    xTaskCreatePinnedToCore(usb_client_task, "ups_usb", 7168, NULL, 20, NULL, 0);
