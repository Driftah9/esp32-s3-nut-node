/*============================================================================
 ...
 - Add guarded candidate tracking for input/output voltage
 - Only publish AC voltages after repeated plausible observations
 ...
 - input.voltage and output.voltage are only promoted after repeated plausible
   values to reduce risk of false mappings
...
static ac_candidate_tracker_t s_input_tracker;
...
static bool tracker_observe(ac_candidate_tracker_t *tr, uint32_t mv)
{
    ...
    /* Treat values within 5V as the same neighborhood for stability */
    if (delta <= 5000U) {
    ...
    switch (rid) {
        ...
        case 0x14:
            if (len >= 3U) {
                ...
                /* Candidate scales to try:
                   - raw * 1V
                   - raw * 0.1V
                   - raw * 0.01V
                   We only publish if plausible and repeated.
                */
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    if (is_plausible_ac_mv(mv)) {
                        ESP_LOGI(TAG,
                                 "candidate output.voltage from report 0x14 raw=%" PRIu32 " scale[%d] => %" PRIu32 " mV",
                                 raw, i, mv);
                        if (tracker_observe(&s_output_tracker, mv)) {
                            upd->output_voltage_valid = true;
                            upd->output_voltage_mv = mv;
                            ESP_LOGI(TAG,
                                     "promoted output.voltage from report 0x14 => %" PRIu32 " mV",
                                     mv);
                            promoted = true;
                        }
                        break;
                ...
                if (!promoted && raw != 0U) {
                    ESP_LOGI(TAG,
                             "report 0x14 raw=%" PRIu32 " not yet plausible for output.voltage",
                             raw);
                }
        ...
        case 0x21:
            if (len >= 2U) {
                ...
                /* Candidate scales to try:
                   ...
                   - raw * 10V
                   - raw * 20V
                   These are chosen because APC units may report compact encoded values.
                */
                uint32_t mv_candidates[3] = {
                    raw * 1000U,
                    raw * 10000U,
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    if (is_plausible_ac_mv(mv)) {
                        ESP_LOGI(TAG,
                                 ...
                                 raw, i, mv);
                        if (tracker_observe(&s_input_tracker, mv)) {
