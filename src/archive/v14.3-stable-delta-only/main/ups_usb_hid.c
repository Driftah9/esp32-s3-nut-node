...
static void set_identity_defaults(void)
{
    ...
    s_product[0] = 0;
    s_serial[0] = 0;
    strlcpy(s_mfr, (s_vid == 0x051d) ? "APC" : "UNKNOWN", sizeof(s_mfr));
...
static void cleanup_device(void)
{
    if (s_dev != NULL) {
        if (s_hid_intf_num >= 0) {
            ...
            if (rel != ESP_OK) ESP_LOGW(TAG, "usb_host_interface_release: %s", esp_err_to_name(rel));
        }
        esp_err_t cerr = usb_host_device_close(s_client, s_dev);
        if (cerr != ESP_OK) ESP_LOGW(TAG, "usb_host_device_close: %s", esp_err_to_name(cerr));
        s_dev = NULL;
    }
...
static void usb_lib_task(void *arg)
{
    ...
    while (1) {
        ...
        if (err != ESP_OK) ESP_LOGE(TAG, "usb_host_lib_handle_events: %s", esp_err_to_name(err));
    }
}
...
static void load_usb_strings(usb_device_handle_t dev)
{
    ...
    if (err != ESP_OK) { ESP_LOGW(TAG, "usb_host_device_info failed: %s", esp_err_to_name(err)); return; }

    if (dev_info.str_desc_manufacturer) usb_str_desc_to_ascii(dev_info.str_desc_manufacturer, s_mfr, sizeof(s_mfr));
    if (dev_info.str_desc_product)      usb_str_desc_to_ascii(dev_info.str_desc_product, s_product, sizeof(s_product));
    if (dev_info.str_desc_serial_num)   usb_str_desc_to_ascii(dev_info.str_desc_serial_num, s_serial, sizeof(s_serial));

    if (s_mfr[0] == 0)     strlcpy(s_mfr, (s_vid == 0x051d) ? "APC" : "UNKNOWN", sizeof(s_mfr));
...
static void log_device_descriptor(usb_device_handle_t dev)
{
    ...
    if (err != ESP_OK || dd == NULL) { ESP_LOGE(TAG, "usb_host_get_device_descriptor failed: %s", esp_err_to_name(err)); return; }
...
static esp_err_t parse_active_config(usb_device_handle_t dev)
{
    ...
    if (err != ESP_OK || cfg == NULL) { ESP_LOGE(TAG, "usb_host_get_active_config_descriptor: %s", esp_err_to_name(err)); return err; }

    ESP_LOGI(TAG, "Active config wTotalLength=%u bNumInterfaces=%u", (unsigned)cfg->wTotalLength, (unsigned)cfg->bNumInterfaces);
    ...
    for (uint16_t i = 0; i + 2U <= total;) {
        ...
        if (bLength == 0U) { ESP_LOGW(TAG, "Descriptor parse: bLength=0 at i=%u (abort)", (unsigned)i); break; }
        if ((uint16_t)(i + bLength) > total) { ESP_LOGW(TAG, "Descriptor parse: overrun at i=%u (abort)", (unsigned)i); break; }
    ...
    if (s_hid_intf_num < 0) { ESP_LOGW(TAG, "No HID interface found"); return ESP_ERR_NOT_FOUND; }
    if (s_ep_in_addr == 0U || s_ep_in_mps == 0U) ESP_LOGW(TAG, "HID interface found but no interrupt IN endpoint");
    return ESP_OK;
}

static void maybe_dump_report_descriptor(usb_device_handle_t dev) { (void)dev; }
...
static void intr_in_cb(usb_transfer_t *t)
{
    ...
    if (t->status == USB_TRANSFER_STATUS_COMPLETED && t->actual_num_bytes > 0U) {
        ...
        bool changed = (len != s_last_in_len) || (memcmp(d, s_last_in, (size_t)len) != 0);
        if (changed) {
            ...
            int n = (len > 64) ? 64 : len;
            for (int i = 0; i < n; i++)
                pos += snprintf(&line[pos], sizeof(line) - (size_t)pos, "%02X%s", d[i], (i == n - 1) ? "" : " ");
        ...
        ESP_LOGW(TAG, "INT-IN transfer status=%d actual=%u", (int)t->status, (unsigned)t->actual_num_bytes);
    }

    if (!s_dev_gone) {
        esp_err_t err = usb_host_transfer_submit(t);
        if (err != ESP_OK) { ESP_LOGW(TAG, "usb_host_transfer_submit(resubmit): %s", esp_err_to_name(err)); usb_host_transfer_free(t); }
    } else {
...
{
    if (s_dev == NULL || s_ep_in_addr == 0U || s_ep_in_mps == 0U) return ESP_ERR_INVALID_STATE;

    usb_transfer_t *t = NULL;
    esp_err_t err = usb_host_transfer_alloc(s_ep_in_mps, 0, &t);
    if (err != ESP_OK || t == NULL) return err;
    ...
    ESP_LOGI(TAG, "Starting interrupt IN reader: EP=0x%02x MPS=%u (change-only)", (unsigned)s_ep_in_addr, (unsigned)s_ep_in_mps);
...
static void usb_client_task(void *arg)
{
    ...
    while (1) {
        ...
        if (s_dev_connected) {
            ...
            if (s_new_dev_addr == 0U) { ESP_LOGW(TAG, "NEW_DEV flagged but addr=0? skip"); continue; }

            usb_device_handle_t dev = NULL;
            esp_err_t err = usb_host_device_open(s_client, s_new_dev_addr, &dev);
            if (err != ESP_OK || dev == NULL) {
                ESP_LOGE(TAG, "usb_host_device_open(addr=%u) failed: %s", (unsigned)s_new_dev_addr, esp_err_to_name(err));
                continue;
            }
            ...
            if (err == ESP_OK) {
                if (s_hid_intf_num >= 0) {
                    err = usb_host_interface_claim(s_client, s_dev, (uint8_t)s_hid_intf_num, (uint8_t)s_hid_alt);
                    if (err == ESP_OK) ESP_LOGI(TAG, "Interface claimed (intf=%d alt=%d)", s_hid_intf_num, s_hid_alt);
                    else ESP_LOGE(TAG, "usb_host_interface_claim failed: %s", esp_err_to_name(err));
                }
                maybe_dump_report_descriptor(s_dev);
#if UPS_USB_ENABLE_INTR_IN_READER
                if (s_ep_in_addr != 0U && s_ep_in_mps != 0U) {
                    esp_err_t rerr = start_interrupt_in_reader();
                    if (rerr != ESP_OK) ESP_LOGW(TAG, "start_interrupt_in_reader: %s", esp_err_to_name(rerr));
                } else {
